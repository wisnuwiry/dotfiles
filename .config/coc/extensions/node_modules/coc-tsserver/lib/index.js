...
var require_cancellation = __commonJS({
  "node_modules/vscode-jsonrpc/lib/common/cancellation.js"(exports) {
    ...
    var CancellationToken27;
    (function(CancellationToken28) {
      CancellationToken28.None = Object.freeze({
        isCancellationRequested: false,
        onCancellationRequested: events_1.Event.None
      });
      CancellationToken28.Cancelled = Object.freeze({
        isCancellationRequested: true,
        onCancellationRequested: events_1.Event.None
      });
      function is(value) {
        const candidate = value;
        return candidate && (candidate === CancellationToken28.None || candidate === CancellationToken28.Cancelled || Is2.boolean(candidate.isCancellationRequested) && !!candidate.onCancellationRequested);
      }
      CancellationToken28.is = is;
    })(CancellationToken27 = exports.CancellationToken || (exports.CancellationToken = {}));
    ...
    var CancellationTokenSource5 = class {
      ...
      cancel() {
        if (!this._token) {
          this._token = CancellationToken27.Cancelled;
        } else {
          this._token.cancel();
        }
      }
      dispose() {
        if (!this._token) {
          this._token = CancellationToken27.None;
...
var TypeScriptCompletionItemProvider = class {
  ...
  async resolveCompletionItem(item, token) {
    ...
    if (!item.data.isSnippet && detail && item.insertTextFormat == import_vscode_languageserver_protocol8.InsertTextFormat.Snippet) {
  ...
  isInValidCommitCharacterContext(document, position) {
    if (this.client.apiVersion.lt(API.v320)) {
      if (position.character > 1) {
        const preText = document.getText(import_vscode_languageserver_protocol8.Range.create(
          position.line,
          0,
          position.line,
          position.character
...
var getSymbolKind2 = (kind) => {
  switch (kind) {
    ...
    case Kind.constructSignature:
    case Kind.constructorImplementation:
      return import_vscode_languageserver_protocol10.SymbolKind.Constructor;
    case Kind.function:
    case Kind.localFunction:
...
var TypeScriptDocumentSymbolProvider = class {
  ...
  static convertSymbol(item, range) {
    ...
    const symbolInfo = import_vscode_languageserver_protocol10.DocumentSymbol.create(
      label,
      "",
...
var _TypeScriptRefactorProvider = class {
  ...
  async provideCodeActions(document, range, context, token) {
    ...
    return this.convertApplicableRefactors(
      ...
      file2,
      range,
      context.only && context.only.some((v) => v.includes(import_vscode_languageserver_protocol14.CodeActionKind.Refactor))
    ).filter((action) => {
...
var BufferSynchronizer = class {
  ...
  flush() {
    ...
    if (this._pending.size > 0) {
      ...
      this.client.execute("updateOpen", { changedFiles, closedFiles, openFiles }, import_vscode_languageserver_protocol15.CancellationToken.None, { nonRecoverable: true });
      this._pending.clear();
    }
  }
...
async function activate(context) {
  ...
  subscriptions.push(import_coc49.services.register(service));
  return {
